...
date& operator+(date& d, int add_days)
{
	...
	static date temp;
	temp.assign(d.day,d.month,d.year);
	int dmax=0;
	
		// logic is to add add_days to the current day then increase the month and 
		// then set current day to the 1. 
		// repeat until sub_days <= 0

	while (add_days > 0)
	{

		if (temp.month == 2) // assigning a maximum to the number of days in a month
		{
		...
		if (temp.month == 12 && (add_days > dmax)) // end of year
		{
			temp.year = temp.year + 1;
			temp.month = 1;
			add_days = add_days - (dmax - temp.day);
			temp.day = 1;
		}
		...
		else if ((temp.day + add_days) > dmax ) // add_days + current day greater than total days in month
		{
			temp.month = temp.month + 1;
			temp.day = (temp.day + add_days) - dmax;
			add_days = 0;
		}

		else if ((temp.day + add_days) <= dmax ) // current day and add_days less than max number of days in month
		{
			temp.day = temp.day + add_days;
			add_days = 0;
...
date& operator-(date& d, int sub_days) // subtract operator
{
	...
	while (sub_days > 0)  // loop until number of days subtracted is 0
	{
		...
		// logic is to subtract current days from sub_days and reduce the month
		//then set current day to the max number of days for the current (-1) month
		// and repeat until sub_days <= 0

		if ((temp.day - sub_days) < 1) // find the start of the month
		{
			
			
			temp.month = temp.month - 1; // go to previous month

			if (temp.month < 1) // go to previous year
			...
			sub_days = sub_days - temp.day; // reduce sub_days by number of days in current month
